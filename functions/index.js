...
exports.weeklyUpdateTopItems = functions.pubsub.schedule('every monday 00:00').timeZone('UTC').onRun(async (context) => {
  try {
    await Promise.all([
      calculateTopReadWeekly(),
      calculateTopPlayedWeekly(),
...
exports.monthlyUpdateTopItems = functions.pubsub.schedule('1 of month 00:00').timeZone('UTC').onRun(async (context) => {
  try {
    await Promise.all([
      calculateTopReadMonthly(),
      calculateTopPlayedMonthly()
...
exports.manualUpdateTopItems = onRequest(async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: `An error occurred during ${type} top items update` });
  }
});
