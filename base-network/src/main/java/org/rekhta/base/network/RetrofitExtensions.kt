...
suspend fun <T, E> Response<T>.toResult(mapper: suspend (T) -> E): Result<E> =
    try {
        if (isSuccessful) {
            ...
            ErrorResult(Exception("Server error: ${toException().message}"))
        }
    } catch (e: Exception) {
        ErrorResult(Exception("Server exception: ${e.message}"))
    }
...
suspend fun <T> resultApiCall(
    ...
    return withContext(dispatcher) {
        try {
            ...
            kotlin.Result.success(apiCall.invoke())
