...
import com.kafka.reader.epub.cache.EpubCache
...
actual class EpubParser(
     ...
     */
    actual suspend fun createEpubBook(filePath: String, shouldUseToc: Boolean): EpubBook {
...
//        val coverImage = parseCoverImage(metadataCoverId, manifestItems, files)
    ...
    private fun getZipFilesFromTempFile(inputStream: InputStream): Map<String, EpubFile> {
        debug { "Copying input stream to a temporary file" }
        val tempFile = File(applicationInfo.cachePath(), "_zip_temp.epub")
        tempFile.outputStream().use { output -> inputStream.copyTo(output) }
        debug { "Input stream copied to temporary file" }
        val epubFile = getZipFilesFromFile(tempFile.absolutePath)
        debug { "EPUB file created from ZipFile" }
        tempFile.delete()
...
//        } else {
//            Log.e("Cover image not found"); null
//        }
//    }
}