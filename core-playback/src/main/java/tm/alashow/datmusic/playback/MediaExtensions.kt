...
fun MediaControllerCompat.playPause() {
    playbackState?.let {
        when {
            it.isPlaying -> transportControls?.sendCustomAction(
                PAUSE_ACTION, bundleOf(BY_UI_KEY to false)
            )
            it.isPlayEnabled -> transportControls?.sendCustomAction(
...
inline val MediaMetadataCompat.contentId: String
    get() = if (containsKey(METADATA_KEY_CONTENT_ID)) getString(METADATA_KEY_CONTENT_ID)
    else ""
